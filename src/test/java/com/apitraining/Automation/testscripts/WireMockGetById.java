...
public class WireMockGetById {
    ...
    public void get() throws URISyntaxException {
        ...
        Response response = RestAssured.given().baseUri("http://localhost:8080").header("Authentication", keyvalue)
                .accept(ContentType.JSON).when().get("/api/wiremockapi/1").then().assertThat().statusCode(200).and()
                .body("employee_name", Matchers.equalTo("nischal")).log().all().extract().response();
